...
* Fix: "Needs to be set up" now verifies every candidate against the post's current verdict, skip flag, and AI state before counting or displaying it. This final check bypasses cached query results and the denormalized status index, so an already-configured or excluded post cannot appear in that view—or be sent back through Draft with AI—even if a hosting cache serves an obsolete candidate list.
* Remove the confusing "X% of readers agreed" line from the post-reveal overlay.
* Add an author dropdown to the bulk swipe setup screen. The selection applies to every status view and to Draft with AI.
* Add page-wide Skip for Now and Never checkboxes to the setup grid, while keeping every row individually adjustable before saving.
* Show each post's author beside its date in the setup grid.
* Add Jetpack's "Views: 30 days" figures to the setup grid, with ascending and descending popularity sorting.
* Fix stale JavaScript and CSS after plugin updates on hosts with long-lived static-file caches. Asset URLs now change whenever their file changes, so new page markup cannot be paired with an older cached script.
