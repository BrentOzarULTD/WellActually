...
class WA_Bulk_Setup {
	...
	private function render_ai_panel( $args ) {
		...
		if ( '' !== $effective && false === WA_AI::model_supports_drafting( $provider_id, $effective ) ) {
			...
			printf(
				/* translators: %s: model id */
				esc_html__( '%s does not support the structured JSON responses drafting needs, so drafting with it is likely to fail. Pick a model that supports JSON/structured output.', 'wellactually' ),
